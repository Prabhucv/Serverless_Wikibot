...
def requestProcess(request):
        ...
        try:
                if request.form.get("text"):
                        data=json.dumps({'slack_url':request.form.get("response_url"),'text':request.form.get("text")}).encode('utf-8') #Creating a packet data for further processing 
        ...
        return jsonify(responseMessage) #Returning the status/temporary response to Slack bot, response should be provided within 3000ms    